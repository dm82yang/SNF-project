...
for (i in 1:space_length){
  ...
  for (i in 1:limit_l){
    ...
    # print position variable and number of patients
    #print(pos)
    #print(length(limit_id))
...
```{r plot result}
colnames(param_df) <- c('snf_type','clust_num','limit_by','sample','response','p')
...
param_df %>%
  ggplot(aes(x = snf_type, y = -log(p), label = limit_by,color = -log(p),size = sample)) + 
