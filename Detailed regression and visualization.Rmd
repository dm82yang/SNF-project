...
plot_df<-regression_df %>%
  mutate(coef = as.numeric(coef),
         ...
         p = as.numeric(p)) %>%
  filter(clust_num==5)%>%
  filter(response %in% c(last_measure,longitudinal_measure,global_measure))
...
plot_df %>%
  ...
  filter(response %in% global_measure) %>%
  ...
  labs(title = "Global Cognition and Rate of Cognitive Decline Associations with Three-Modality Subtypes", x = '', y = "Regression Coefficients") +
  annotate("text", x = "Global Cognition", y = 0.14, label="adj p-val = 0.02", color = "darkcyan",size=3.2)+
